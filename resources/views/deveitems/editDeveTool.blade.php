...
<form method="POST" action="{{ route('devetool.update', ['devetool' => $deveTool->id]) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <input type="text" id="toolName" name="toolName" value="{{ $deveTool->tool_name }}" required>

    <!-- Developer tool Description Input -->
    <div class="form-group">
        <label for="toolDescription">Developer tool description:</label>
        <textarea class="form-control mt-1 mb-3" id="toolDescription" name="toolDescription" >{{ $deveTool->tool_description }}</textarea>
    </div>
    ...
    @if($deveTool->image_path)
        <img src="{{ asset('storage/' . $deveTool->image_path) }}" class="img-fluid p-4" alt="Current Image">
        
    @endif

    <!-- Submit Button -->
    <button type="submit" class="btn btn-outline-primary mt-2">Update Developer Tool</button>
    <!-- Cancel Button -->
