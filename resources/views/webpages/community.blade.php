    ...
    <!-- Feedback Form -->
            <div class="row mx-0">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12  header_items_box">
                    <h2 class="text-center font-semibold">CodeFlexi Blog </h2>
                    <p class="text-md text-center my-2">Welcome to the friendliest place for anyone learning to code! Start creating posts.</p>
                </div>
            </div>
</div>
...
<div class="container">
        ...
        @if(in_array(Auth::user()->role, ['member', 'admin']))
            ...
            <div class="row mt-1 mx-0">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12  align-self-center">
                    <div class="card post-form" style="max-width: 45rem; border:none; box-shadow:none; border-radius:10px;">
                        <div class="card-body">

                            <form method="POST" action="{{ route('post.store') }}" enctype="multipart/form-data">
                                @csrf
                                <div class="form-group my-1 mx-4">
                                    <label for="feedbackTitle" class="mt-4">Post Title:</label>
                                    <input type="text" id="feedbackTitle" name="title"  value="{{ old('title') }}" required>
                                </div>
                                <div class="form-group my-3 mx-4">
                                    <label for="PostMessage" class="">Post Message:</label>
                                    <textarea class="form-control mt-1 mb-3" id="PostMessage" name="message" placeholder="Ask or share something, this is your space too">{{ old('message') }}</textarea>
                                </div>

                                <div id="drop-area" class="drop-area my-3 mx-4" onclick="chooseImage()">
                                    <input type="file" id="image" name="image" style="display: none;" onchange="displayFileName(); clearError();" />
                                    <label class="btn-img" for="image">Choose Image</label>
                                    <div class="text-sm mt-2" id="file-name">TO UPLOAD WITH POST</div>
                                    @error('image')
                                        <span class="text-danger" id="error-message">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="form-group mt-4 mx-4">
                                    <button type="submit" class="btn btn-outline-primary w-100 orange-btn-outline-primary"><i class="fa fa-paper-plane" ></i>&nbsp;Post</button>
                                </div>
                            </form>
 ...
 @endauth
            ...
            <div class="row mt-4 mx-0">
                ...
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 mx-0 align-self-center">
                    <h3 class="text-center mb-4" style=" font-size:25px;font-weight:500 !important;color:#1385d1 !important;"> All Members Posts</h3>
                <div>
                        ...
                        <div class="text-center d-flex justify-content-center mb-4">
                            <div class="dropdown" style="max-width: 16rem; "> 
                                ...
                                <ul class="dropdown-menu" aria-labelledby="sortDropdown" style="width: 13rem ; font-size:15px !important; font-family: 'Fira Code', monospace;">
                                    <li><a class="dropdown-item" href="{{ route('community', ['sort' => 'new']) }}">New Posts</a></li>
                                    <li><a class="dropdown-item" href="{{ route('community', ['sort' => 'old']) }}">Old Posts</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
...
@endif
    ...
    @endforeach
@else
    <p class="text-center lead">There are no posts yet</p>
@endif
</div>
...
<script>
    function toggleComments(postId) {
        ...
        if (commentsDiv.style.display === 'none' && toggleBtn.innerText.includes('Show')) {
            ...
            toggleBtn.style.color = '#0072BB'; // Set the color to blue when comments are hidden
        } else {
            commentsDiv.style.display = 'none';
            toggleBtn.innerHTML = toggleBtn.innerHTML.replace('Hide', 'Show');
            toggleBtn.style.color = ''; // Reset the color when comments are shown
        }
