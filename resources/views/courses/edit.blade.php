<head>
        ...
        <title>CodeFlexi | course edit</title>
        {{-- Bootstrap 5 links --}}
...
@endif
             ...
             <h3 class="display-5 text-center">Edit Course Page</h3>
...
<div class="form-group mb-3">
    ...
    <select class="form-control" id="catalog" name="catalog">
        ...
        @foreach ($catalogs as $catalog)
            <option value="{{ $catalog->id }}" {{ $course->catalog_id == $catalog->id ? 'selected' : '' }}>
...
<label for="courseTitle" class="mt-4">Course Title:</label>
    <input type="text" id="courseTitle" name="courseTitle" value="{{ $course->course_name }}" required>

    <div class="form-group">
        <label for="courseDescription">Course Description:</label>
        <textarea class="form-control mt-1 mb-3" id="courseDescription" name="courseDescription" >{{ $course->course_description }}</textarea>
    </div>
...
<div class="form-group">
    ...
    <div class="form-group mt-3">
        ...
        <input type="number" class="form-control" id="timeLong" name="timeLong" value="{{ $course->time_long }}">
    </div>

<!-- Hidden input to ensure a value is always sent for 'complete' -->
<input type="hidden" name="complete" value="0">

<!-- Checkbox input -->
<div class="form-check">
    <input type="checkbox" class="form-check-input" id="complete" name="complete" {{ $course->complete == 1 ? 'checked' : '' }}>
    ...
    @if($course->image_path)
        <img src="{{ asset('storage/' . $course->image_path) }}" class="img-fluid p-4" alt="Current Image">
        
    @endif

    <!-- Submit Button -->
    <button type="submit" class="btn btn-outline-primary mt-2">Update Course</button>
    <!-- Cancel Button -->
