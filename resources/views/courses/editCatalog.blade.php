<head>
        ...
        <title>CodeFlexi | Catalog edit</title>
        {{-- Bootstrap 5 links --}}
...
@endif
             ...
             <h3 class="display-5 text-center">Edit Catalog Page</h3>
...
<form method="POST" action="{{ route('catalog.update', ['catalog' => $catalog->id]) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <input type="text" id="catalogName" name="catalogName" class="form-control" value="{{ $catalog->catalog_name }}" required>
           
    <div class="form-group">
        <label for="catalogDescription">Catalog Description:</label>
        <textarea class="form-control" id="catalogDescription" name="catalogDescription" >{{ $catalog->catalog_description }}</textarea>
    </div>
    


    <!-- Submit Button -->
    <button type="submit" class="btn btn-outline-primary mt-2">Update Catalog</button>
    <!-- Cancel Button -->
